...
var patient = ({
    ...
    submitRawData: function (data) {
        ...
        data.data.patient_id = patient.patientId;
    ...
    submitData: function (url) {
        ...
        var args = {
            ...
            names: {
                family_name: (data.data["Last Name"] ? data.data["Last Name"] : null),
                given_name: (data.data["First Name"] ? data.data["First Name"] : null),
                middle_name: (data.data["Middle Name"] ? data.data["Middle Name"] : null)
            },
            gender: (data.data["Gender"] ? data.data["Gender"] : null),
            attributes: {
                occupation: (data.data["Occupation"] ? data.data["Occupation"] : null),
                cell_phone_number: (data.data["Cellphone Number"] ? data.data["Cellphone Number"] : null)
            },
            birthdate: (data.data["Date of birth"] ? data.data["Date of birth"] : null),
            patient: {
                identifiers: {}
            },
            birthdate_estimated: (data.data["Birthdate Estimated"] ? data.data["Birthdate Estimated"] : null),
            addresses: { current_residence: (data.data["Closest Landmark"] ? data.data["Closest Landmark"] : null),
                current_village: (data.data["Current Village"] ? data.data["Current Village"] : null),
                current_ta: (data.data["Current T/A"] ? data.data["Current T/A"] : null),
                current_district: (data.data["Current District"] ? data.data["Current District"] : null),
                home_village: (data.data["Home Village"] ? data.data["Home Village"] : null),
                home_ta: (data.data["Home T/A"] ? data.data["Home T/A"] : null),
                home_district: (data.data["Home District"] ? data.data["Home District"] : null)
            },
            status: 'NEW RECORD',
            action: "UPDATE"
        };

        patient.jsonOriginalRecord = args;

        patient.doSubmitData(args, url);

    },

    doSubmitData: function (args, url) {

        patient.ajaxAuthPostRequest(url, args, function (result) {
            ...
            if (json.status == "OK") {
                ...
                if (json.canPrintBarcode) {
                    ...
                    patient.printBarcode(npid, function () {
            ...
            } else if (json.length > 1 || (json[0].status == "POSSIBLE MATCH" || json[0].status == "MULTIPLE") &&
    ...
    verifyMatch: function (json, url) {
        ...
        form.setAttribute("action", "javascript:patient.submitSearch()");
    ...
    loadVerificationNames: function () {
        ...
        newPatient.onmousedown = function () {
            ...
            patient.doSubmitData(patient.jsonOriginalRecord, patient.settings.ddePath + patient.settings.savePath);
        ...
        editPatient.innerHTML = "<span>Update Selected</span>";
        ...
        editPatient.onmousedown = function () {
            ...
            if (patient.jsonMatches.length > 0) {
                ...
                if (patient.jsonMatches[pos]) {
                    ...
                    patient.jsonMatches[pos].status = "SELECTED";
    ...
    loadVerificationDetails: function () {
        ...
        if (Object.keys(patient.jsonMatches).length == 0) {
            patient.$("rightpanel").innerHTML = "<div style='margin: auto; padding: 20px; font-size: 24px; font-style: italic;'>No results found!</div>";
        }
        ...
        for (var i = 0; i < patient.jsonMatches.length; i++) {
            ...
            div.onclick = function () {
                ...
                patient.patientId = this.getAttribute("patient_id");
    ...
    showVerificationPatient: function (pos) {
        ...
        cell2_2.innerHTML = (patient.jsonMatches[pos]["birthdate_estimated"] ? "~" : "") +
            patient.getAge(patient.jsonMatches[pos]["birthdate"]);
    ...
    ajaxSearch: function (page, url) {
        ...
        /*var url = patient.settings.ddePath + patient.settings.searchPath + "/?first_name=" + patient.$("first_name").value.trim() +
         "&last_name=" + patient.$("last_name").value.trim() + "&gender=" +
         patient.$("gender").value.trim() + "&page=" + page;*/
    ...
    loadNames: function () {
        ...
        editPatient.onmousedown = function () {
            ...
            patient.buildEditPage(this.getAttribute("patient_id"));
                ...
                for (var j = 0; j < 4; j++) {
                    ...
                    switch (j) {
                        ...
                        case 2:
                            ...
                            var url = patient.settings.ddePath + patient.settings.demographicsQueryByIdentifierPath +
                                patient_id + "/?field=" + field + "&token=" + patient.getCookie("token");
                            ...
                            patient.ajaxAuthRequest(url, function (result) {
                                ...
                                if (patient.$(field)) {
                                    ...
                                    if (field == "date_of_birth") {
                                        ...
                                        patient.$(field).innerHTML = (json[field + "_estimated"] == "1" ? "~" : "") +
    ...
    buildEditFieldPage: function (field, field_target_id, field_target_id_value) {
            ...
            if (patient.settings.demographics[fields[i]] || ["Current Region", "Region of Origin"].indexOf(fields[i]) >= 0) {
                ...
                switch (fields[i]) {
                    ...
                    case "Current T/A":
                        ...
                        input['ajaxURL'] = patient.settings.ddePath + patient.settings.taQueryPath + "?district=&ta=";
                    ...
                    case "Current Village":
                        ...
                        input['ajaxURL'] = patient.settings.ddePath + patient.settings.villageQueryPath + "?district=&ta=&village=";
                    ...
                    case "Region of Origin":
                        ...
                        var targetId = (fields[i] == "Current Region" ? "current_district" : "home_district");

                        input['tt_onUnload'] = "if(__$('data." + targetId + "')){var url = __$('data." + targetId +
                            "').getAttribute('ajaxURL').replace(/region\=[^&]+|region=&/,'region='+__$('touchscreenInput'" +
                            "+tstCurrentPage).value.trim()+'&'); __$('data." +
                            targetId + "').setAttribute('ajaxURL', url);} ";
                    ...
                    case "Date of birth":
                        ...
                        input['field_type'] = "birthdate";

                        input['estimate_field'] = "birthdate_estimated";

                        input['estimate_field'] = "date_of_birth_estimated";

                        var birthdateEstimated = {
                            type: "hidden",
                            id: "date_of_birth_estimated",
                            name: "data.date_of_birth_estimated",
                            value: 0
                        }
                ...
                if (fields[i] == "Gender") {
                    ...
                    input.options = ["Southern Region", "Northern Region", "Central Region"];
        ...
        patient.navPanel(form.outerHTML);
    ...
    navPanel: function (content) {
        ...
        if (patient.$("patient.navPanel")) {
            ...
            var html = "<html><head><title></title><base href='" + base + "' /> <script type='text/javascript' language='javascript' " +
                ...
                "if(window.parent) window.parent.patient.submitRawData(data); }</script></head><body>";
    ...
    buildAddPage: function (first_name, last_name, gender) {
        ...
        form.id = "patient.form.main";
        form.style.display = "none";
        form.setAttribute("action", "javascript:patient.submitData('" + patient.settings.ddePath + patient.settings.savePath + "')");
            ...
            if (patient.settings.demographics[fields[i]] || ["Current Region", "Region of Origin"].indexOf(fields[i]) >= 0) {
                ...
                switch (fields[i]) {
                    ...
                    case "Current District":
                        ...
                        input.setAttribute("ajaxURL", patient.settings.ddePath + patient.settings.districtQueryPath + "?region=&district=");

                        var targetId = (fields[i] == "Current District" ? "current_t_a" : "home_t_a");

                        var targetId2 = (fields[i] == "Current District" ? "current_village" : "home_village");

                        input.setAttribute("tt_onUnload",
                                "if(patient.$('data." + targetId + "')){var url = patient.$('data." + targetId +
                                "').getAttribute('ajaxURL').replace(/district\=[^&]+|district=&/,'district='+" +
                                "patient.$('touchscreenInput'+tstCurrentPage).value.trim()+'&'); patient.$('data." +
                                targetId + "').setAttribute('ajaxURL', url)}; if(patient.$('data." + targetId2 +
                                "')){var url = patient.$('data." + targetId2 + "').getAttribute('ajaxURL').replace(" +
                                "/district\=[^&]+|district=&/,'district='+patient.$('touchscreenInput'+" +
                                "tstCurrentPage).value.trim()+'&'); patient.$('data." + targetId2 +
                                "').setAttribute('ajaxURL', url);} patient.$('nextButton').style.display = 'none';");

                        input.setAttribute("tt_requireNextClick", false);

                        input.setAttribute("tt_pageStyleClass", "StandardSelectList");
                    ...
                    case "Current T/A":
                        ...
                        input.setAttribute("ajaxURL", patient.settings.ddePath + patient.settings.taQueryPath + "?region=&district=&ta=");

                        var targetId = (fields[i] == "Current T/A" ? "current_village" : "home_village");

                        input.setAttribute("tt_onUnload",
                                "if(patient.$('data." + targetId + "')){var url = patient.$('data." + targetId +
                                "').getAttribute('ajaxURL').replace(/ta\=[^&]+|ta=&/,'ta='+patient.$('touchscreenInput'+" +
                                "tstCurrentPage).value.trim()+'&'); patient.$('data." +
                                targetId + "').setAttribute('ajaxURL', url);} patient.$('nextButton').style.display = 'none';");
                    ...
                    case "Current Village":
                        ...
                        input.setAttribute("ajaxURL", patient.settings.ddePath + patient.settings.villageQueryPath + "?region=&district=&ta=&village=");
                    ...
                    case "Region of Origin":
                        ...
                        var targetTa = (fields[i] == "Current Region" ? "current_t_a" : "home_t_a");

                        var targetVillage = (fields[i] == "Current Region" ? "current_village" : "home_village");

                        input.setAttribute("tt_onUnload",
                                "if(patient.$('data." + targetDistrict + "')){var url = patient.$('data." + targetDistrict +
                                "').getAttribute('ajaxURL').replace(/region\=[^&]+|region=&/,'region='+patient.$('touchscreenInput'" +
                                "+tstCurrentPage).value.trim()+'&'); patient.$('data." +
                                targetDistrict + "').setAttribute('ajaxURL', url);}; if(patient.$('data." + targetTa +
                                "')){var url = patient.$('data." + targetTa +
                                "').getAttribute('ajaxURL').replace(/region\=[^&]+|region=&/,'region='+patient.$('touchscreenInput'" +
                                "+tstCurrentPage).value.trim()+'&'); patient.$('data." +
                                targetTa + "').setAttribute('ajaxURL', url);}; if(patient.$('data." + targetVillage +
                                "')){var url = patient.$('data." + targetVillage +
                                "').getAttribute('ajaxURL').replace(/region\=[^&]+|region=&/,'region='+patient.$('touchscreenInput'" +
                                "+tstCurrentPage).value.trim()+'&'); patient.$('data." +
                        ...
                        input.setAttribute("tt_onLoad", "patient.$('nextButton').style.display = 'none';");
                    ...
                    case "Nationality":
                        ...
                        input.setAttribute("ajaxURL", patient.settings.ddePath + patient.settings.nationalityQueryPath + "?nationality=");
                    ...
                    case "Middle Name":
                        ...
                        input.setAttribute("tt_onLoad", " " +
                            "patient.$('data.first_name').removeAttribute('condition'); " +
                            "patient.$('data.last_name').removeAttribute('condition'); " +
                            "patient.$('data.gender').removeAttribute('condition')");
                    ...
                    case "Office Phone Number":
                        ...
                        input.setAttribute("tt_pageStyleClass", "NumbersWithUnknown nota");

                        input.setAttribute("validationRule", "^0\\d{7}$|Unknown|Not Available|^0\\d{9}$|^N\\/A$");

                        input.setAttribute("validationMessage", "Not a valid phone number");

                        input.setAttribute("field_type", "number");
                    ...
                    case "Date of birth":
                        ...
                        input.setAttribute("field_type", "birthdate");

                        input.setAttribute("estimate_field", "birthdate_estimated");
                        ...
                        var birthdateEstimated = document.createElement("input");
                        birthdateEstimated.type = "hidden";
                        birthdateEstimated.id = "birthdate_estimated";
                        birthdateEstimated.name = "data.Birthdate Estimated";
                        birthdateEstimated.value = 0;

                        td2.appendChild(birthdateEstimated);
                ...
                if (fields[i] == "Gender") {
                    ...
                    var opts = ["Southern Region", "Northern Region", "Central Region"];
